...
router.post("/", (req, res, next) => {
  ...
  try {
    ...
    // }
    if (!author) {
      const exception = new Error(`Missing author field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!country) {
      const exception = new Error(`Missing country field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!imageLink) {
      const exception = new Error(`Missing imageLink field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!language) {
      const exception = new Error(`Missing language field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!pages) {
      const exception = new Error(`Missing pages field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!title) {
      const exception = new Error(`Missing title field`);
      exception.statusCode = 401;
      throw exception;
    }
    if (!year) {
      const exception = new Error(`Missing year field`);
      exception.statusCode = 401;
      throw exception;
    }
